...
import { CheckCircle2, Clock3, XCircle, ClipboardCheck, User, Video } from "lucide-react";
import {
  DoctorAppointment,
  DoctorDecision,
...
export default function DoctorAppointmentManager() {
  ...
  const handleJoinVideoCall = async (appointment: DoctorAppointment) => {
    setJoiningCall(true);
    try {
      let session;
      try {
            ...
            {selectedAppointment ? (
                <>
                  ...
                  {selectedAppointment.appointmentType === "VIDEO" ? (
                      <button
                          type="button"
                          onClick={() => void handleJoinVideoCall(selectedAppointment)}
                          disabled={joiningCall}
                          className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-blue-600 px-4 py-3 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-60"
                      >
