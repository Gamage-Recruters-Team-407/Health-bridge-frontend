...
export default function AppointmentsPage() {
  ...
  return <AppointmentModuleShell title="My Appointments" subtitle="Track upcoming channeling sessions, history, and live queue numbers." action={<Link href="/appointments/search-doctor" className="rounded-2xl bg-blue-600 px-5 py-3 text-sm font-semibold text-white">Find a Session</Link>}>
    ...
    : <div className="space-y-4">{visible.map(a=>{const today=a.date===new Date().toISOString().slice(0,10);const ahead=Math.max(a.appointmentNumber-a.currentQueueNumber-1,0);const canCancel=canCancelAppointment(a);return <article key={a.appointmentId} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-center"><div><div className="flex flex-wrap items-center gap-3"><h2 className="text-lg font-bold text-slate-900">{a.doctorName}</h2><span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-bold text-blue-700">{a.status}</span></div><p className="mt-1 text-sm text-blue-700">{a.specialization} · {a.hospitalName}</p><div className="mt-3 flex flex-wrap gap-x-5 gap-y-2 text-sm text-slate-500"><span>Ref: {a.referenceNumber}</span><span>No. {String(a.appointmentNumber).padStart(2,"0")}</span><span>{a.date} at {a.sessionTime}</span>{today&&["BOOKED","UPCOMING"].includes(a.status)&&<span className="font-semibold text-emerald-700">Current {String(a.currentQueueNumber).padStart(2,"0")} · {ahead} ahead</span>}</div>{["BOOKED","UPCOMING"].includes(a.status)&&!canCancel&&<p className="mt-2 text-xs font-medium text-amber-700">Cancellation is unavailable within 24 hours of the session.</p>}</div><div className="flex flex-wrap gap-2"><Link href={`/appointments/${a.appointmentId}`} className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700">View</Link>{["BOOKED","UPCOMING"].includes(a.status)&&<><Link href={`/appointments/${a.appointmentId}`} className="rounded-xl border border-blue-200 px-4 py-2 text-sm font-semibold text-blue-700">Reschedule</Link><button disabled={!canCancel} title={canCancel?"Cancel appointment":"Appointments can only be cancelled at least 24 hours before the session."} onClick={()=>setCancel(a)} className="rounded-xl border border-rose-200 px-4 py-2 text-sm font-semibold text-rose-700 disabled:cursor-not-allowed disabled:border-slate-200 disabled:text-slate-400">Cancel</button></>}</div></div></article>})}</div>}
