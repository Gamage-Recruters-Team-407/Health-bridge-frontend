...
function AppointmentDetailsContent({ params }: AppointmentDetailsPageProps) {
    ...
    const handleJoinVideoCall = async () => {
        ...
        try {
            let session;
            try {
                session = await telemedicineApi.getSessionByAppointmentId(appointment.id);
            } catch {
                // No session exists yet for this appointment — create one now.
            ...
            setError(getErrorMessage(err, "Unable to start the video consultation. Please try again."));
        } finally {
