...
import { CalendarDays, CircleDollarSign, Clock3, Stethoscope, UserRound, UsersRound, PlaneTakeoff, ArrowRight, Video } from "lucide-react";
...
import { mockAvailability, mockDoctors, mockEarnings } from "@/features/doctor/constants";
import { getDoctorProfile } from "@/features/doctor/services/doctorService";
import type { Doctor } from "@/features/doctor/types";

export default function DoctorDashboardPage() {
  const [doctor, setDoctor] = useState<Doctor>(mockDoctors[0]);
  ...
  const schedule = mockAvailability.filter((slot) => slot.status === "Available").slice(0, 4);
  return <><PageHeader eyebrow="Monday, August 24" title={`Good morning, ${doctor.fullName.replace("Dr. ", "Dr. ")}`} description="Here is your clinical overview and schedule for today." />
